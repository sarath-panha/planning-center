    ...
    <section className="py-20">
      <MaxWidthWrapper>
        <div className="bg-primary rounded-[3rem] p-12 lg:p-20 text-center space-y-8 relative overflow-hidden shadow-2xl shadow-primary/40">
          ...
          <Cpu className="h-16 w-16 text-white/20 mx-auto relative z-10" />
          <h2 className="text-3xl md:text-5xl font-bold text-white relative z-10">Industrial Production on Demand</h2>
          <p className="text-xl text-white/80 max-w-2xl mx-auto relative z-10">
            Professional 5-axis milling and 3D printing for dental labs and clinics. High turnaround and rapid shipping.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 relative z-10">
            <Button size="xl" variant="secondary" className="rounded-full font-bold" asChild>
              <Link href="/portal/start">Launch Production Case</Link>
            </Button>
            <Button size="xl" variant="outline" className="rounded-full text-white border-white/30 hover:bg-white/10" asChild>
              <Link href="/contact">Request Material Samples</Link>
            </Button>
          </div>
        </div>
      </MaxWidthWrapper>
